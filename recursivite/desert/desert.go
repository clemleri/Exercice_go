...
On notera que, si la personne trouve plus de 33 unités d'eau par nuit, 
alors elle peut survivre infiniment longtemps dans le désert. 
Ce cas ne sera jamais considéré dans les tests 
(le paramètre eauTrouvee sera toujours compris entre 0 et 33). 
Dans tous les autres cas, la personne finira par mourir de soif.

# Entrées
- eau, la quantité d'eau dont dispose initialement la personne dans sa gourde
- eauTrouvee, la quantité d'eau que la personne trouve chaque nuit

# Sortie
- jours, le nombre de jours entiers que peut tenir la personne avant de mourir de soif

# Info
2022-2023, test 2, exercice 5
*/

func duree(eau, eauTrouvee int) (jours int) {
	if eau<100{
		if eau+2*eauTrouvee<100 {
			return 2
		}else {
			return duree(eau-100+2*eauTrouvee,eauTrouvee)+2
		}
	}else {
		return duree(eau-100+eauTrouvee,eauTrouvee)+1
	}
}