...
La fonction trouveCycle doit retourner un entier qui représente les décimales qui se répètent dans le résultat de la division d'un entier x par un entier y.
Elle retournera toujours le moins de décimales possibles (par exemple pour 7/11 on retournera 63 et pas 6363) et les décimales représentant le cycle
dès qu'il commence (par exemple pour 7/11 on retournera 63 et pas 36).

Il est possible que le décimales qui se répètent ne puissent pas se représenter avec un entier (car il y en a trop). 
On considérera que c'est le cas quand il y en a 18 ou plus. Dans ce cas, il faudra retourner l'erreur errCycleTropLong
